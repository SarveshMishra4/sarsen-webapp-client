    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {activityFeed.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-6 bg-gray-50 rounded-lg">No activity yet.</p>
        ) : (
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., Q4 Strategy Document" />
            </div>
            <div>
              ...
              <input type="url" value={formData.url} onChange={e => setFormData({ ...formData, url: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="https://..." />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })}
                rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Brief description of this resource..." />
            </div>
      ...
      <div className="space-y-3">
        {files.length > 0 ? (
          files.map(file => (
            <div key={file._id} className="bg-white rounded-lg p-4 border border-gray-200 hover:shadow-md transition-all">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="flex items-start gap-3 flex-1">
                  <div className={`p-2 rounded-lg ${RESOURCE_COLORS[file.fileType] ?? 'bg-gray-100 text-gray-700'}`}>
                ...
                <div className="flex gap-2 self-end sm:self-start">
                  <a href={file.fileUrl} target="_blank" rel="noopener noreferrer"
                    className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Open Resource">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                  </a>
                  <button onClick={() => handleDeleteResource(file._id)}
                    className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete Resource">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={e => setFormData(f => ({ ...f, description: e.target.value }))}
                rows={2} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
            ...
            <div className="bg-white rounded-lg p-4 border-2 border-dashed border-gray-300">
              ...
              <div className="space-y-3">
                <input type="text" value={newQuestion.question} onChange={e => setNewQuestion(q => ({ ...q, question: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm" placeholder="Enter your question..." />
      ...
      <div className="space-y-3">
        {questionnaires.length > 0 ? (
          questionnaires.map(q => (
            <div key={q._id} className="bg-white rounded-lg p-6 border border-gray-200">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => handleDeleteQuestionnaire(q._id)}
                  className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete Questionnaire">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                </button>
              </div>
                ...
                qAddingTo === q._id ? (
                  <div className="flex gap-2 mt-4">
                    ...
                    <button onClick={() => handleAddInlineQuestion(q._id)} disabled={!inlineQ.trim()}
                      className="px-4 py-2 bg-purple-600 text-white text-sm rounded-lg disabled:opacity-40">Add</button>
                    <button onClick={() => { setQAddingTo(null); setInlineQ(''); }}
                      className="px-4 py-2 bg-gray-100 text-gray-600 text-sm rounded-lg">Cancel</button>
                  </div>
                ) : (
            ...
            <>
              <div className="flex flex-col sm:flex-row gap-3">
                <textarea value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={handleKeyPress}
                  rows={2} className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            ...
            {selected.status !== 'delivered' && selected.canDeliver && (
              <button onClick={deliverEngagement} disabled={delivering}
                className="text-sm bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50 self-start flex items-center gap-2">
        ...
        <div className="p-6">
          ...
          {activeSubTab === 'brief' && (
            <div>
              ...
              {purchaseAnswers.length === 0 ? (
                <div className="text-center text-gray-400 py-10 bg-gray-50 rounded-lg">No purchase answers recorded.</div>
              ) : (
                <div className="space-y-3">
                  {purchaseAnswers.map((a, i) => (
              ...
              {resetPasswordText && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(resetPasswordText)} className="px-3 py-2 bg-amber-500 text-white text-sm rounded-lg hover:bg-amber-600 transition-colors">Copy</button>
                  </div>
                    ...
                    {users.map(u => (
                      <div key={u._id} className="px-6 py-4 flex items-center justify-between">
                        ...
                        <button onClick={() => handleResetPassword(u._id)} disabled={resetUserId === u._id}
                          className="px-3 py-1.5 bg-red-50 border border-red-200 text-red-600 text-xs rounded-lg hover:bg-red-100 transition-colors disabled:opacity-50">
            ...
            <div className="mt-8 border-t border-gray-100 pt-6">
              ...
              <div className="space-y-2">
                {selected.engagementChecklist.slice().sort((a, b) => a.order - b.order).map(step => (
                  <div key={step.stepId} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
