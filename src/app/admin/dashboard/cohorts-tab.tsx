...
export function CohortsTab({
  ...
  cohorts: Cohort[];
  setCohorts: React.Dispatch<React.SetStateAction<Cohort[]>>;
}) {
  ...
  const handleViewEnrolled = (cohort: Cohort) => {
    setShowEnrolledModal(cohort);
  };
        ...
        {cohorts.map(cohort => (
          <div key={cohort.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            {editingCohort?.id === cohort.id ? (
              // Edit mode
              <EditCohortForm cohort={cohort} onSave={handleSaveEdit} onCancel={handleCancelEdit} />
            ) : (
              // View mode
              <>
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <button
                    onClick={() => handleViewEnrolled(cohort)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => setShowEnrolledModal(null)}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="flex gap-2 pt-2">
        <button type="submit" className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg text-sm">
          Save
        </button>
        <button type="button" onClick={onCancel} className="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 py-2 rounded-lg text-sm">
          Cancel
        </button>
      </div>
    </form>
