...
export function BlogsTab({
  ...
  const handleSave = () => {
    ...
    if (editingPost) {
      ...
      setBlogPosts((prev) => prev.map((post) => (post.id === editingPost.id ? { ...post, ...formData } as BlogPost : post)));
    } else {
      // Create new post
  ...
  const handlePublish = (id: string) => {
    setBlogPosts((prev) =>
      prev.map((post) =>
        post.id === id ? { ...post, status: 'published' as const, publishedAt: new Date().toISOString() } : post
      )
    );
  };
          ...
          <div key={post.id} className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              ...
              <div className="flex gap-2 self-end sm:self-start">
                <button onClick={() => handleEdit(post)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
